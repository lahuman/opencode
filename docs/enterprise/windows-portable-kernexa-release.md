# Kernexa: Windows Portable ZIP Release

This runbook is the only supported operator path from a reviewed source revision to Windows acceptance for the Kernexa portable release. It produces an unsigned Windows x64 ZIP and its integrity artifacts; it does not install the application, register a protocol, migrate data from the former Pilot application, or remove user data.
...
```text
dist\kernexa-<version>-win-x64.zip
dist\kernexa-<version>-win-x64.zip.sha256
dist\kernexa-<version>-win-x64.release.json
dist\kernexa-<version>-win-x64.sbom.cdx.json
dist\kernexa-<version>-win-x64.third-party-licenses.txt
```
...
```powershell
$archive = @(Get-ChildItem .\dist\kernexa-*-win-x64.zip -File)
...
if ($archive.Name -notmatch '^kernexa-.+-win-x64\.zip$') {
...
bun run smoke:enterprise:portable -- `
  -Archive .\dist\kernexa-1.17.18-win-x64.zip `
  -Checksum .\dist\kernexa-1.17.18-win-x64.zip.sha256 `
  -ReleaseMetadata .\dist\kernexa-1.17.18-win-x64.release.json `
...
try {
  ...
  $executables = @(Get-ChildItem -LiteralPath $signatureRoot -Recurse -File -Filter "Kernexa.exe")
...
- [ ] Replacing the extracted application folder preserves settings and credentials under `%LOCALAPPDATA%\com.company.kernexa`.
...
To roll back Kernexa, distribute the last accepted complete ZIP/checksum/metadata set through the trusted internal channel, verify its hash and metadata compatibility before extraction, and replace only the extracted Kernexa application folder. Do not delete `%LOCALAPPDATA%\com.company.kernexa`, `%LOCALAPPDATA%\com.company.opencode.pilot`, or project directories during rollback. Kernexa settings and DPAPI-protected credentials must remain available after replacement and after deleting an extracted folder; former Pilot data remains independent and untouched.
