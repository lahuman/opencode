...
```text
dist\sfmi-<version>-win-x64.zip
dist\sfmi-<version>-win-x64.zip.sha256
dist\sfmi-<version>-win-x64.release.json
dist\sfmi-<version>-win-x64.sbom.cdx.json
dist\sfmi-<version>-win-x64.third-party-licenses.txt
```
...
```powershell
$archive = @(Get-ChildItem .\dist\sfmi-*-win-x64.zip -File)
...
if ($archive.Name -notmatch '^sfmi-.+-win-x64\.zip$') {
...
bun run smoke:enterprise:portable -- `
  -Archive .\dist\sfmi-1.17.18-win-x64.zip `
  -Checksum .\dist\sfmi-1.17.18-win-x64.zip.sha256 `
  -ReleaseMetadata .\dist\sfmi-1.17.18-win-x64.release.json `
