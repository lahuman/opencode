# SFMI Enterprise Help Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the enterprise-only Tabs promotion with Korean SFMI onboarding and rename/version every active enterprise guide surface as `SFMI AI 가이드` with `버전 sfmi-1`.
...
- Enterprise compact-card title is exactly `SFMI 시작하기`.
- Enterprise compact-card description is exactly `코드 분석부터 구현과 검증까지 한곳에서 진행하세요.`.
- Enterprise drawer title is exactly `SFMI AI Coding Workspace`.
- Enterprise guide name is exactly `SFMI AI 가이드`.
- Enterprise guide version identifier is exactly `sfmi-1`, rendered as `버전 sfmi-1`.
...
### Task 1: SFMI guide labels across active UI surfaces
...
- Produces: user-facing guide label `SFMI AI 가이드` and dialog version prefix `버전` without changing command or IPC identifiers.
...
```ts
page.getByRole("menuitem", { name: "SFMI AI 가이드" })
page.getByRole("button", { name: "SFMI AI 가이드 열기" })
dialog.getByRole("region", { name: "SFMI AI 가이드 내용" })
dialog.getByText("SFMI AI 가이드", { exact: true })
dialog.getByText("버전 sfmi-1", { exact: true })
```
...
Expected: failures show `Company AI Guide` instead of `SFMI AI 가이드`.
...
Use `SFMI AI 가이드` in the enterprise desktop menu. Use `SFMI AI 가이드 열기` for the floating help button and error-page accessible names, while visible error-page text is `SFMI AI 가이드`.
...
git commit -m "feat(app): brand SFMI help guide"
```
...
- Produces: `SFMI_ONBOARDING`, a readonly data object consumed by `TabsInfoPopup`.
...
import { SFMI_ONBOARDING } from "./help-content"
...
export const SFMI_ONBOARDING = {
  ...
  drawer: {
    ...
    closeLabel: "닫기",
    title: "SFMI AI Coding Workspace",
    intro: "SFMI는 저장소의 맥락을 이해하고 필요한 변경을 구현한 뒤 결과를 근거로 검증하는 AI 코딩 작업공간입니다.",
    sections: [
...
In `help-button.tsx`, import `SFMI_ONBOARDING`. Branch the compact card labels, text, and media on `platform.enterprise`. The enterprise media is:

```tsx
<img src={homeImage} alt="" class="absolute inset-0 h-full w-full object-cover" />
```

The public fallback remains the existing `introducingTabsVideo` element and exact English strings. In the drawer, render `SFMI_ONBOARDING.drawer` for enterprise and retain the existing English paragraphs/images as the fallback. Map the three enterprise sections with visible Korean headings and descriptions.
...
git commit -m "feat(app): add Korean SFMI onboarding"
```

### Task 3: Guide resource and `sfmi-1` version contract
...
- Modify: `docs/enterprise/windows-portable-sfmi-release.md`
...
- Produces: guide version `sfmi-1` consistently across runtime fixtures, release examples, the checked-in manifest, and the UI guide payload.

- [ ] **Step 1: Change guide-specific expectations to `sfmi-1`**
...
Expected: failures show `pilot-1` where the updated expectations require `sfmi-1`.
...
In `docs/enterprise/windows-portable-sfmi-release.md`, set:
...
Expected: `resources/enterprise/enterprise-manifest.json` contains `"guideVersion": "sfmi-1"` and a new SHA-256 for `company-guide.md`. Preserve the pre-existing `modelCatalogSHA256`, model IDs, defaults version, catalog version, default model, and allowed origins if the local build profile is not reproducible from repository sources.
...
Expected: enterprise guide selectors find `SFMI AI 가이드`, `SFMI AI 가이드 내용`, and `버전 sfmi-1`; public help assertions remain unchanged.
...
```powershell
git commit -m "chore(desktop): version SFMI guide"
```
...
- Verify: `docs/enterprise/windows-portable-sfmi-release.md`
...
```powershell
rg -n "SFMI 시작하기|SFMI AI Coding Workspace|Introducing Tabs|introducingTabsVideo" packages/app/src/components/help-button.tsx packages/app/src/components/help-content.ts
```

Expected: SFMI and public Tabs content are both present.
...
```powershell
git diff --check
git diff -- packages/app packages/desktop/resources/enterprise docs/enterprise/windows-portable-sfmi-release.md
```
