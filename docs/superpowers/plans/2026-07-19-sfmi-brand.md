# SFMI Brand Implementation Plan
...
**Goal:** Replace the active closed-network desktop product identity with SFMI while preserving ordinary OpenCode behavior and keeping the former Pilot application and data fully separate.
...
**Global Constraints:** Enterprise-only changes must not alter ordinary OpenCode identities. SFMI uses `com.company.sfmi`, `SFMI.exe`, `sfmi-${version}-win-x64.*`, and `%LOCALAPPDATA%\com.company.sfmi`. No code may read, migrate, copy, or remove `%LOCALAPPDATA%\com.company.opencode.pilot`. Do not edit historical dated plans/specifications. Run tests and type checks from `packages/desktop`, never from the repository root.
...
- app ID: `com.company.sfmi`
- product/window/document title: `SFMI`
- artifact pattern: `sfmi-${version}-${os}-${arch}.${ext}`
- user-data directory: `%LOCALAPPDATA%\com.company.sfmi`
...
Change fixtures and expectations to require `SFMI.exe` and `sfmi-${version}-win-x64.zip`, including related-file naming and Authenticode target selection.
...
Change archive construction, executable lookup, required-file checks, related metadata lookup, and size/integrity checks to SFMI names without weakening verification.
...
Require the `sfmi` SBOM component, SFMI notice heading, SFMI artifact sidecars, `SFMI.exe`, and `%LOCALAPPDATA%\com.company.sfmi`.
...
- Rename: `docs/enterprise/windows-portable-pilot-release.md` to `docs/enterprise/windows-portable-sfmi-release.md`
- Modify: `docs/enterprise/windows-portable-sfmi-release.md`
- Modify: `packages/desktop/scripts/windows-portable-smoke.test.ts`

### Step 1: Change the runbook contract test

Point the test at the SFMI runbook and require current SFMI executable, artifact, sidecar, and AppData examples.
...
Expected: the SFMI runbook does not yet exist or still contains Pilot instructions.

### Step 3: Rename and update the active runbook

Rename the file and update all active operator instructions to SFMI. Retain operational controls and rollback guidance, but make clear that Pilot data is neither migrated nor deleted.
...
```powershell
git diff --check
git diff -- packages/desktop docs/enterprise docs/superpowers/plans/2026-07-19-sfmi-brand.md
```
