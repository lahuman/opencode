...
A read-only agent designed for implementation planning. Plan can inspect and search the project, research documentation, and ask focused questions, but it cannot run shell commands, edit files, invoke subagents, or execute custom action tools.

In Desktop, the guided Plan workflow saves the final Markdown to `.opencode/plans` for Git projects or the global data directory for other projects, then asks whether to switch to Build. Saving this plan artifact is the only write performed by Plan mode. Approving the prompt switches to Build and starts implementation; choosing **Keep planning** leaves the session in Plan mode. CLI users can enable this workflow with `OPENCODE_EXPERIMENTAL_PLAN_MODE=true`.

Use `/plan` to toggle Plan mode and `/build` to switch directly to Build. Desktop displays the current mode as **Build** or **Plan · Read-only** in the composer.
