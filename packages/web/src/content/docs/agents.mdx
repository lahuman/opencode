...
Desktop and CLI show the final `plan_exit` tool input as the authoritative Markdown plan, then ask whether to switch to Build. Approving the prompt saves a compatibility copy to `.opencode/plans` for Git projects or the global data directory for other projects, switches to Build, and starts implementation. Choosing **Keep planning** or dismissing the prompt leaves the session in Plan mode without writing a file.
