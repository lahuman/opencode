...
Desktop and CLI show the final `plan_exit` tool input as the authoritative Markdown plan, then show a **Build now** or **Keep planning** confirmation card. Choosing **Build now** starts a Build turn using the approved plan. Presenting a plan never writes a file, but Build starts from a synthetic implementation request.
