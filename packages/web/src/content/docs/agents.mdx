...
Desktop and CLI show the final `plan_exit` tool input as the authoritative Markdown plan and end the planning turn without starting implementation. Plan mode remains active so you can refine the plan. To implement it, explicitly switch to Build and send an implementation request; presenting a plan never writes a file or creates a synthetic Build message.
