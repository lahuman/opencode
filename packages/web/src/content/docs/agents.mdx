...
Desktop and CLI show the final `plan_exit` tool input as the authoritative Markdown plan, then show a **Build now** or **Keep planning** confirmation card. Choosing **Build now** switches the composer to Build without starting implementation. After switching, send an implementation request. Presenting a plan never writes a file or creates a synthetic Build message.
