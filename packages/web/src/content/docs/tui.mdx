...
Toggle the approval-gated Plan agent. Plan can inspect and search the project, ask questions, and request user-approved shell commands for read-only inspection or project-documented tests and type checks. Its final tool input is the authoritative conversation plan. A compatibility copy is saved only after you approve switching to Build. You can switch agents with the `Tab` key.
