...
Switch to the Build agent. New prompts can use tools and edit files according to your configured permissions.
...
Toggle the approval-gated Plan agent. Plan can inspect and search the project, ask questions, and request user-approved shell commands for read-only inspection or project-documented tests and type checks. Its final tool input is the authoritative conversation plan, and presenting it never starts implementation. Stay in Plan to refine it, or switch to Build with `Tab` or `/build` and send an implementation request.
