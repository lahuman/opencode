...
Toggle the read-only Plan agent. Plan can inspect and search the project and ask questions. With `OPENCODE_EXPERIMENTAL_PLAN_MODE=true`, it also saves a final Markdown plan before requesting approval to switch to Build. You can switch agents with the `Tab` key.
