...
it.live("always approvals stay scoped to one directory", () =>
  Effect.gen(function* () {
    const one = yield* tmpdirScoped({ git: true })
    const two = yield* tmpdirScoped({ git: true })
    const store = yield* InstanceStore.Service
    const sourceID = PermissionV1.ID.make("per_always_directory_source")
    const source = yield* store
      .provide(
        { directory: one },
        ask({
          id: sourceID,
          sessionID: SessionID.make("session_always_directory_source"),
          permission: "bash",
          patterns: ["ls"],
          metadata: {},
          always: ["ls"],
    ...
    expect(yield* store.provide({ directory: one }, waitForPending(1))).toHaveLength(1)
    yield* store.provide({ directory: one }, reply({ requestID: sourceID, reply: "always" }))
    yield* Fiber.join(source)
    ...
    expect(
      yield* store.provide(
        { directory: one },
        ask({
          id: PermissionV1.ID.make("per_always_directory_same"),
          sessionID: SessionID.make("session_always_directory_same"),
          permission: "bash",
          patterns: ["ls"],
          metadata: {},
          always: [],
    ...
    const isolated = yield* store
      .provide(
        { directory: two },
        ask({
          id: PermissionV1.ID.make("per_always_directory_other"),
          sessionID: SessionID.make("session_always_directory_other"),
          permission: "bash",
          patterns: ["ls"],
          metadata: {},
          always: [],
      ...
      .pipe(Effect.forkScoped)
    const pending = yield* store.provide({ directory: two }, waitForPending(1))
    expect(pending[0].id).toBe(PermissionV1.ID.make("per_always_directory_other"))
    yield* store.provide({ directory: two }, reply({ requestID: pending[0].id, reply: "reject" }))
    expect(Exit.isFailure(yield* Fiber.await(isolated))).toBe(true)
  }),
)

it.live("always approvals clear on instance reload", () =>
  Effect.gen(function* () {
    const directory = yield* tmpdirScoped({ git: true })
    const store = yield* InstanceStore.Service
    const sourceID = PermissionV1.ID.make("per_always_reload_source")
    const source = yield* store
      .provide(
        { directory },
        ask({
          id: sourceID,
          sessionID: SessionID.make("session_always_reload_source"),
          permission: "bash",
          patterns: ["ls"],
          metadata: {},
          always: ["ls"],
    ...
    expect(yield* store.provide({ directory }, waitForPending(1))).toHaveLength(1)
    yield* store.provide({ directory }, reply({ requestID: sourceID, reply: "always" }))
    yield* Fiber.join(source)
    expect(
      yield* store.provide(
        { directory },
        ask({
          id: PermissionV1.ID.make("per_always_reload_before"),
          sessionID: SessionID.make("session_always_reload_before"),
          permission: "bash",
          patterns: ["ls"],
          metadata: {},
          always: [],
    ...
    const after = yield* store
      .provide(
        { directory },
        ask({
          id: PermissionV1.ID.make("per_always_reload_after"),
          sessionID: SessionID.make("session_always_reload_after"),
          permission: "bash",
          patterns: ["ls"],
          metadata: {},
          always: [],
      ...
      .pipe(Effect.forkScoped)
    const pending = yield* store.provide({ directory }, waitForPending(1))
    expect(pending[0].id).toBe(PermissionV1.ID.make("per_always_reload_after"))
    yield* store.provide({ directory }, reply({ requestID: pending[0].id, reply: "reject" }))
    expect(Exit.isFailure(yield* Fiber.await(after))).toBe(true)
  }),
)
