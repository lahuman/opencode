...
for (const name of ["config.json", "opencode.json", "opencode.jsonc"]) {
  it.live(`enterprise ${name} updates preserve V2 settings while removing provider secrets`, () =>
    withProcessEnvs(
      ...
      withGlobalConfig(
        {
          name,
          config: {
            agents: { reviewer: { system: "Keep the company review instructions" } },
            provider: {
              company: {
                options: { baseURL: "https://llm.corp.example/v1", apiKey: "fake-existing-secret" },
                models: {
                  code: {
                    headers: { Authorization: "fake-existing-secret" },
                    options: { temperature: 0, key: "fake-existing-secret" },
                  },
                },
              },
            },
        ...
        ({ dir }) =>
          withInstanceDir(
            ...
            Effect.gen(function* () {
              const patch = {
                username: "updated",
                provider: { company: { options: { headers: { Authorization: "fake-new-secret" } } } },
              }
              yield* name === "config.json"
                ? Config.use.update(patch).pipe(Effect.asVoid)
                : Config.use.updateGlobal(patch).pipe(Effect.asVoid)
              const written = yield* FSUtil.use.readFileString(path.join(dir, name))
              expect(written).not.toContain("fake-existing-secret")
              expect(written).not.toContain("fake-new-secret")
              expect(ConfigParse.jsonc(written, name)).toMatchObject({
                username: "updated",
                agents: { reviewer: { system: "Keep the company review instructions" } },
                provider: {
                  company: {
                    options: { baseURL: "https://llm.corp.example/v1" },
                    models: { code: { options: { temperature: 0 } } },
                  },
                },
              })
            }),
