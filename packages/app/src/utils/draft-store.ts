...
function parseJSON(value: string, reviver?: (key: string, value: unknown) => unknown) {
  try {
...
export function createDraftStore(driver: Driver): DraftStore {
  ...
  return {
    getItem: async (key) => {
      ...
      return parsed === undefined ? value : JSON.stringify(await decode(parsed))
    },
  ...
  const db = new Promise<IDBDatabase>((resolve, reject) => {
    request.addEventListener("success", () => {
      ...
      documents.addEventListener("success", () => {
        ...
        const parsed = parseJSON(`[${documents.result.join(",")}]`, (_key, item) => {
          if (item && typeof item === "object" && "blob" in item) {
            const blob = item.blob
            if (blob && typeof blob === "object" && "id" in blob && typeof blob.id === "string") used.add(blob.id)
          }
          return item
        })
